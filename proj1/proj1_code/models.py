...
class HybridImageModel(nn.Module):
  ...
  def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
    ...
    matrix = np.zeros((self.n_channels, 1, kernel_2d.shape[0], kernel_2d.shape[1]))

    for i in range(self.n_channels):
      matrix[i][0] = kernel_2d
  ...
  def forward(self, image1, image2, cutoff_frequency):
    ...
    high_frequencies = image2.sub(self.low_pass(image2, kernel))
    hybrid_image = low_frequencies.add(high_frequencies)
