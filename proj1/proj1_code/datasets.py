...
def make_dataset(path: str) -> Tuple[List[str],List[str]]:
  ...
  for filename in os.listdir(path):
    ...
    if filename_segments[0][-1] == 'a':
      images_a.append(os.path.join(path, filename))
    elif filename_segments[0][-1] == 'b':
...
def get_cutoff_frequencies(path: str) -> List[int]:
  ...
  for i in range(len(file)):
    cutoff_frequencies.append(int(file[i]))
...
class HybridImageDataset(data.Dataset):
  ...
  def __getitem__(self, idx: int) -> Tuple[torch.Tensor,torch.Tensor,int]:
    ...
    # THIS IS ALSO NOT NECESSAR
    # image_a = np.transpose(image_a, (2, 0, 1))
    # image_b = np.transpose(image_b, (2, 0, 1))

    # THE TRANSFORM TOTENSOR NORMALIZES AND TRANSPOSES IT FOR YOU
