...
class ImageLoader(data.Dataset):
  ...
  def load_imagepaths_with_labels(self, class_labels) -> List[Tuple[str, int]]:
    ...
    # for root, dirs, files in os.walk(self.curr_folder):
    #   for file in files:
    #     if file.endswith('.jpg'):
    #         img_paths.append((os.path.join(root, file), class_labels[root.split('/')[-1]]))

    for label in class_labels:
      for g in glob.glob(self.curr_folder + "/" + label + "/*.jpg"):
        img_paths.append((g, class_labels[label]))
    ###########################################################################
    # Student code end
    ###########################################################################
    return img_paths
  ...
  def get_classes(self) -> dict:
    ...
    files = os.listdir(self.curr_folder)
  ...
  def __getitem__(self, index: int) -> Tuple[torch.tensor, int]:
    ...
    # #print('info: ', info)
    # class_idx = info[1]
    # img = self.load_img_from_path(info[0])
    # if self.transform:
