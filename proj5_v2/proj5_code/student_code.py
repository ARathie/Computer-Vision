...
def get_tiny_images(image_arrays):
    ...
    for image in image_arrays:
        resized = cv2.resize(image, (new_dim, new_dim))
        flattened_image = np.ndarray.flatten(resized)
        normalized = (flattened_image - flattened_image.mean())/ flattened_image.std()
...
def nearest_neighbor_classify(train_image_feats, train_labels,
    ...
    for i in range(len(test_image_feats)):
        distances_from_train = distances[i][:]
        closest = []
        for _ in range(k):
            index, value = min(enumerate(distances_from_train), key=operator.itemgetter(1))
            closest.append(train_labels[index])
            distances_from_train[index] = sys.maxsize
        closest_label = max(set(closest), key = closest.count) 
        test_labels.append(closest_label)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def kmeans(feature_vectors, k, max_iter = 100):
    ...
    ftrs = feature_vectors.tolist()
    centroids = []
    curr_centroid = 0
    while k > curr_centroid:
        dim = feature_vectors.shape[0]
        var = np.random.randint(0, dim, 1)
        if (ftrs[var[0]] not in centroids):
            curr_centroid += 1
            centroids.append(ftrs[var[0]])

    centroids = np.array(centroids)

    for _ in range(0, max_iter):
        var2 = []
        centroid_idx = kmeans_quantize(feature_vectors, centroids)
        for i in range(len(centroids)):
            centroid_points = np.where(centroid_idx == i)
            mean = np.mean(feature_vectors[centroid_points], axis = 0)
            var2.append(mean)
        centroids = np.array(var2)
...
def build_vocabulary(image_arrays, vocab_size, stride = 20):
    ...
    for curr_image in image_arrays:
        height, width = curr_image.shape
        np_arr_w = np.arange(start = 10, stop = width - 10, step = stride)
        nparr_h = np.arange(start = 10, stop = height - 10, step = stride)

        h_grid, w_grid = np.meshgrid(nparr_h, np_arr_w)
        w_grid_flat = w_grid.flatten()
        h_grid_flat = h_grid.flatten()
        h_num_ftrs = len(h_grid_flat)

        idx = [random.randrange (0, h_num_ftrs, 1) for i in range(h_num_ftrs//2)]
        curr_image = np.array(curr_image, dtype='float32')
        img_tensor = torch.from_numpy(curr_image).reshape((1, 1, curr_image.shape[0], curr_image.shape[1]))

        sift_ftrs = get_siftnet_features(img_tensor, w_grid_flat[idx], h_grid_flat[idx])
        for feat in sift_ftrs:
            list_ftrs.append(feat)

    vocab = kmeans(np.array(list_ftrs), vocab_size, max_iter = 50)
...
def get_bags_of_sifts(image_arrays, vocabulary, step_size = 10):
    ...
    ftr = np.zeros((num_images, vocab_size))
    ...
    for image in image_arrays:
        ...
        nparr_width = np.arange(start = 10, stop = width - 10, step = step_size)
        nparr_height = np.arange(start = 10, stop = height - 10, step = step_size)

        hgrid, wgrid = np.meshgrid(nparr_height, nparr_width)
        wgrid_flat = wgrid.flatten()
        hgrid_flat = hgrid.flatten()

        num_ftrs_h = len(hgrid_flat)
        idx = [random.randrange (0, num_ftrs_h, 1) for i in range(0, num_ftrs_h//2)]

        image = np.array(image, dtype='float32')
        img_tensor = torch.from_numpy(image)
        img_tensor = img_tensor.reshape((1, 1, image.shape[0], image.shape[1]))
        ftr = get_siftnet_features(img_tensor, wgrid_flat[idx], hgrid_flat[idx])

        quantized = kmeans_quantize(ftr, vocab)
        count = np.bincount(quantized, minlength = vocab_size)
