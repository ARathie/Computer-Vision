...
class HarrisNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.net = nn.Sequential(image_gradients_layer, ChannelProductLayer(), SecondMomentMatrixLayer(), CornerResponseLayer(), NMSLayer())
...
class ChannelProductLayer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_dims = x.shape
        output = torch.zeros(x_dims[0], 3, x_dims[2], x_dims[3])
        for image_num in range(x_dims[0]):
            Ix = x[image_num, 0, :, :]
            Iy = x[image_num, 1, :, :]
            Ixx = torch.mul(Ix, Ix)
            Iyy = torch.mul(Iy, Iy)
            Ixy = torch.mul(Ix, Iy)
            output[image_num][0] = Ixx
            output[image_num][1] = Iyy
            output[image_num][2] = Ixy
...
class SecondMomentMatrixLayer(torch.nn.Module):
    ...
    def __init__(self, ksize: torch.Tensor = 7, sigma: torch.Tensor = 5):
        ...
        self.conv_method = nn.Conv2d(in_channels=3, out_channels=3, kernel_size=self.ksize,
            bias=False, padding=self.ksize//2, padding_mode='zeros', groups=3)

        kernel = get_gaussian_kernel(self.ksize, self.sigma)
        kernel = kernel.view(1, ksize, ksize)
        kernel = torch.stack((kernel, kernel, kernel), dim=0)
        self.conv_method.weight = nn.Parameter(kernel)
...
class CornerResponseLayer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_dims = x.shape        
        output = torch.zeros(x_dims[0], 1, x_dims[2], x_dims[3])
        for image_num in range(x_dims[0]):
            a = Sxx = x[image_num][0]
            d = Syy = x[image_num][1]
            c = b = Sxy = x[image_num][2]

            det = torch.sub(torch.mul(a, d), torch.mul(b, c))
            trace = torch.add(a, d)
            score = det - torch.mul(trace**2, self.alpha)
            output[image_num][0] = score
...
class NMSLayer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        med = torch.median(x)
        larger_vals = torch.where((x >= med), x, zeros)
        pool = nn.MaxPool2d(7, stride=1, padding=3)
        deduped_tensor = pool(larger_vals)
        output = torch.where((x == deduped_tensor), x, zeros)
...
def get_interest_points(image: torch.Tensor, num_points: int = 4500) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    nonzero_inds = torch.nonzero(R, as_tuple=True)
    mask = torch.gt(R, 0.0)
    nonzero_R = torch.masked_select(R, mask)
    sorted_R_indices = torch.argsort(nonzero_R, descending=True)
    
    x = nonzero_inds[3]
    y = nonzero_inds[2]
    confidences = nonzero_R
    
    x = x[sorted_R_indices]
    y = y[sorted_R_indices]
    confidences = confidences[sorted_R_indices]
...
def remove_border_vals(img, x: torch.Tensor, y: torch.Tensor, c: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    while x.size()[0] > ind_count:
        valid = x[ind_count] > 6  and y[ind_count] > 6 and y[ind_count] < dim1 - 7 and x[ind_count] < dim2 - 7
        if valid:
            new_x.append(x[ind_count])
            new_y.append(y[ind_count])
            new_c.append(c[ind_count])
        ind_count += 1

    x = torch.as_tensor(new_x)
    y = torch.as_tensor(new_y)
    c = torch.as_tensor(new_c)
