...
class HistogramLayer(nn.Module):
	...
	def forward(self, x) -> torch.Tensor:
		...
		** You will receive extra-credit if you successfully implement this
		...
		#######################################################################
		dims = x.shape
		per_px_histogram = torch.zeros((1, 8, dims[2], dims[3]))

		grid1, grid2 = np.meshgrid(np.arange(dims[2]), np.arange(dims[3]))

		stream1 = grid1.flatten()
		stream2 = grid2.flatten()
		vals = cosines[0, :, stream1, stream2]
		highest_val, inds = torch.max(vals, dim=0)
		channel1_sq = im_grads[0, 0, stream1, stream2] ** 2
		channel2_sq = im_grads[0, 1, stream1, stream2] ** 2
		matched = torch.sqrt(channel1_sq + channel2_sq)
		per_px_histogram[0, :, stream1, stream2] = torch.where(highest_val == vals, matched, torch.zeros(1))
...
class SubGridAccumulationLayer(nn.Module):
	...
	def __init__(self) -> None:
		...
		self.layer = nn.Conv2d(8, 8, 4, padding=2,  stride=1, bias=False, groups=8)
		dims = (8, 1, 4, 4)
		self.layer.weight = nn.Parameter(torch.ones(dims))
...
class SIFTOrientationLayer(nn.Module):
	...
	def __init__(self):
		...
		self.layer = nn.Conv2d(bias=False, groups=1, in_channels=2, out_channels=10, kernel_size=1)
	...
	def get_orientation_bin_weights(self) -> torch.nn.Parameter():
		...
		vectors = angles_to_vectors_2d_pytorch(radians)

		varx = torch.Tensor([1, 0]).reshape(1, 2)
		vary = torch.Tensor([0, 1]).reshape(1, 2)

		weight_param = torch.cat((vectors, varx, vary), 0).reshape(10, 2, 1, 1)
	...
	def forward(self, x: torch.Tensor) -> torch.Tensor:
		...
		"""
		self.layer.weight = nn.Parameter(self.get_orientation_bin_weights())
		z = self.layer(x)
...
def get_sift_subgrid_coords(x_center: int, y_center: int):
	...
	y = np.int64(y_center - 6)
	for _ in range(4):
		x = np.int64(x_center - 6)
		for _ in range(4):
			x_grid.append(np.int64(x))
			x += 4
			y_grid.append(np.int64(y))
		y += 4
...
def get_siftnet_features(img_bw: torch.Tensor, x: np.ndarray, y: np.ndarray) -> np.ndarray:
	...
	fvs = np.zeros((dims_x[0], 128))

	for m in range(dims_x[0]):
	    x_grid, y_grid = get_sift_subgrid_coords(x[m], y[m])
	    arr = np.zeros((0))
	    for n in range(x_grid.shape[0]):
	        var = image[0, :, y_grid[n], x_grid[n]]
	        arr = np.concatenate((arr, var.detach().numpy()), axis=None)

	    operated = sum(arr ** 2)
	    operated = operated ** (1.0 / 2)
	    fvs[m, :] = (arr / operated) ** 0.9
