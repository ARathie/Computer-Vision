...
def calculate_num_ransac_iterations(prob_success, sample_size, ind_prob_correct):
    ...
    denominator = math.log(1 - math.pow(ind_prob_correct,sample_size))
...
def find_inliers(x_0s, F, x_1s, threshold):
    ...
    """
    ##############################
    M = fundamental_matrix.signed_point_line_errors(x_0s, F, x_1s)
    inliers = []
    for point_line in range(0,len(M),2):
        if M[point_line] < threshold and M[point_line + 1] < threshold:
            inliers.append(point_line//2)
...
def ransac_fundamental_matrix(x_0s, x_1s):
    ...
    x0_concat, x1_concat = np.concatenate((x_0s, np.ones((len(x_0s), 1))), axis=1), np.concatenate((x_1s, np.ones((len(x_1s), 1))), axis=1)

    minavgE = 99999
    for _ in range(number_of_iteration):
        rand_sample = np.random.choice(len(x_0s), 9, replace=False)
        x0_process, x1_process = two_view_data.preprocess_data(x_0s[rand_sample], x_1s[rand_sample])
        F = solve_F(x_0s[rand_sample], x_1s[rand_sample])
        found_inliers = find_inliers(x0_process, F, x1_process, 1)

        if len(found_inliers) >= 0:
            errors = fundamental_matrix.signed_point_line_errors(x0_concat[found_inliers], F, x1_concat[found_inliers])
            for i in range(len(errors)):
                errors[i] = abs(errors[i])
            average_error = sum(errors) / len(errors)
            if average_error < minavgE:
                minavgE, best_F, inliers_x_0, inliers_x_1 = average_error, F, x_0s[found_inliers], x_1s[found_inliers]
    ##############################
