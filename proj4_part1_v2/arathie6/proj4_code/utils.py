...
def generate_random_stereogram(im_size: Tuple[int, int, int] = (51, 51, 3), disparity: int = 4) -> torch.Tensor:
  ...
  init_left = torch.randint(2, (H,W))
  im_left = torch.stack([init_left]*C, dim = 2)
  im_right = im_left.clone()
  dim_vert = math.floor(H/2)
  dim_hor = math.floor(W/2)
  patch_window = im_right[dim_vert-math.floor(dim_vert/2):dim_vert+math.floor(dim_vert/2)+1, dim_hor-math.floor(dim_hor/2):dim_hor+math.floor(dim_hor/2)+1, :]
  im_right[dim_vert-math.floor(dim_vert/2):dim_vert+math.floor(dim_vert/2)+1, dim_hor-math.floor(dim_hor/2)-disparity:dim_hor+math.floor(dim_hor/2)+1-disparity, :] = patch_window
  im_right[dim_vert-math.floor(dim_vert/2):dim_vert+math.floor(dim_vert/2)+1, dim_hor+math.floor(dim_hor/2)+1-disparity:dim_hor+math.floor(dim_hor/2)+1, :] = torch.stack([torch.randint(2, (dim_vert,disparity))]*C, dim = 2)
  im_left = im_left.to(torch.float)
  im_right = im_right.to(torch.float)
  ############################################################################
  # Student code begin
  ############################################################################
