...
def signed_point_line_errors(x_0s, F, x_1s):
    ...
    ##############################
    # TODO: Student code goes here
    x0_arr = np.asarray(x_0s)
    d_list = []
    x1_arr = np.asarray(x_1s)
    
    for i in range(len(x_0s)):
        # Process x1
        Fx_var1 = np.dot(F,x1_arr[i])
        d_var1 = point_line_distance(Fx_var1,x0_arr[i])
        d_list.append(d_var1)

        F_trans = F.T

        # Process x2
        Fx_var2 = np.dot(F_trans,x0_arr[i])
        d_var2 = point_line_distance(Fx_var2,x1_arr[i])
        d_list.append(d_var2)
    errors = d_list
